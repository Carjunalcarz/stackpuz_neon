...
exports.index = (req, res, next) => {
  ...
  let query = knex('Product')
    ...
    .select('Product.id as "id"', 'Product.image as "image"', 'Product.name as "name"', 'Product.price as "price"', 'Brand.name as "brand_name"', 'UserAccount.name as "user_account_name"')
...
exports.get = (req, res, next) => {
  let sqlProduct = knex('Product')
    .leftJoin('Brand', 'Product.brand_id', 'Brand.id')
    .leftJoin('UserAccount', 'Product.create_user', 'UserAccount.id')
    .select('Product.id as "id"', 'Product.name as "name"', 'Product.price as "price"', 'Brand.name as "brand_name"', 'UserAccount.name as "user_account_name"', 'Product.image as "image"')
...
exports.edit = (req, res, next) => {
  let sqlProduct = knex('Product')
    .select('Product.id as "id"', 'Product.name as "name"', 'Product.price as "price"', 'Product.brand_id as "brand_id"', 'Product.image as "image"')
    .where('Product.id', req.params.id)
    .toString()
  let sqlBrand = knex('Brand')
    .select('Brand.id as "id"', 'Brand.name as "name"')
