...
exports.index = (req, res, next) => {
  ...
  let query = knex('OrderHeader')
    ...
    .select('OrderHeader.id as "id"', 'Customer.name as "customer_name"', 'OrderHeader.order_date as "order_date"')
...
exports.get = (req, res, next) => {
  ...
  let sqlOrderHeaderOrderDetail = knex('OrderHeader')
    ...
    .select('OrderDetail.no as "no"', 'Product.name as "product_name"', 'OrderDetail.qty as "qty"')
    .where('OrderHeader.id', req.params.id)
    .toString()
  Promise.all([
...
exports.edit = (req, res, next) => {
  let sqlOrderHeader = knex('OrderHeader')
    .select('OrderHeader.id as "id"', 'OrderHeader.customer_id as "customer_id"', 'OrderHeader.order_date as "order_date"')
    .where('OrderHeader.id', req.params.id)
    .toString()
  let sqlOrderHeaderOrderDetail = knex('OrderHeader')
    .join('OrderDetail', 'OrderHeader.id', 'OrderDetail.order_id')
    .join('Product', 'OrderDetail.product_id', 'Product.id')
    .select('OrderDetail.no as "no"', 'Product.name as "product_name"', 'OrderDetail.qty as "qty"', 'OrderDetail.order_id as "order_id"')
    .where('OrderHeader.id', req.params.id)
    .toString()
  let sqlCustomer = knex('Customer')
    .select('Customer.id as "id"', 'Customer.name as "name"')
